...
POST   /users/me/chats           { title? }     → { thread }
GET    /users/me/chats/{id}                     → { thread, messages[] }
PATCH  /users/me/chats/{id}      { title }      → { thread }
...
Refs are shareable referral links that users create to track signups. Each ref gets a unique 6-character slug. Content is checked by AI for safety before acceptance.

**Public (No Auth):**
```
GET    /refs/{ref_slug}                         → { ref }
```
...
GET    /users/me/refs/{ref_slug}                → { ref }             Get ref detail
PATCH  /users/me/refs/{ref_slug}                → { ref }             Update ref
GET    /users/me/refs/{ref_slug}/referrals ?cursor= → { referrals[], pending[], pagination? }
```
...
```typescript
// Response (200) — who signed up via this ref
{
  success: true,
  referrals: Array<{ user_id, email_blurred, created_at, active_voucher_slug? }>,
  pending: Array<{ user_id, email_blurred: "Pending User", created_at, active_voucher_slug? }>,
  pagination?: { cursor }
}
```
`referrals` = converted users. `pending` = vouchers sent but user hasn't signed up yet. Emails are blurred (e.g. `j***n@gmail.com`).

**Referral tracking at signup:** Pass `ref` in the OTP verification call to attribute the signup:
```
POST /auth/verify-otp  { email, code, ref?: string }
```
...
```typescript
{
  ...
  documentation?: string,          // API docs for AI agent (max 500000 chars)
...
```typescript
if (install.allowed_until !== -1 && Date.now()/1000 >= install.allowed_until) {
...
```typescript
async function settleAndMintCredits(reservationId, installId, installSecret, amount, final, userId) {
  const result = await fetch(`https://cloud.officex.app/v1/reservations/${reservationId}/settle`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-officex-install-id": installId, "x-officex-install-secret": installSecret },
...
const reservation = await reserve({ amount: 50, job_id: `session-${sessionId}` });
await settleAndMintCredits(reservation.reservation_id, installId, installSecret, 50, true, userId);
...
module.exports = {
  async headers() {
    return [{
      source: "/:path*",
      headers: [{ key: "Content-Security-Policy", value: "frame-ancestors 'self' https://officex.app https://*.officex.app" }],
    }];
  },
};
```
...
app.use((req, res, next) => {
  res.setHeader("Content-Security-Policy", "frame-ancestors 'self' https://officex.app https://*.officex.app");
  next();
});
```
...
app.post("/webhooks/officex", async (req, res) => {
  ...
  if (event === "INSTALL") {
    ...
    if (!user) {
      user = await db.users.create({ officex_user_id: user_id, officex_install_id: install_id, created_at: new Date() });
    } else {
      await db.users.update({ officex_user_id: user_id }, { officex_install_id: install_id });
    }
    // Return agent_context — credentials the AI agent needs to call your API
    res.json({ agent_context: { user_token: user.apiToken, base_url: "https://myapp.com/api/v1" } });
  } else {
...
app.get("/officex", async (req, res) => {
  ...
  req.session.officex = { install_id: installId, install_secret: req.query.officex_install_secret };
...
async function resolveUser(installId, customerId) {
  ...
  return await db.users.create({ officex_user_id: customerId, officex_install_id: installId, created_at: new Date() });
}
```
...
async function safeReserve(installId, installSecret, amount, jobId) {
  try {
    ...
    if (!res.success) { console.error(`OfficeX reserve failed: ${res.error?.code}`, res.error); return res; }
    return res;
  } catch (err) {
    console.error("OfficeX API unreachable, queuing for retry:", err.message);
    await billingRetryQueue.enqueue({ installId, amount, jobId, attemptedAt: new Date() });
    return { success: false, error: { code: "OFFICEX_UNREACHABLE", message: err.message } };
  }
}
```
...
async function reserveWithRetry(installId, amount, jobId) {
  try {
    ...
    if (error.code === "INSTALL_EXPIRED") {
      await sendInboxMessage(installId, { id: `renew-${jobId}`, title: "Authorization Expired", text: "Please renew your authorization to continue." });
    }
    if (error.code === "INSUFFICIENT_FUNDS") {
      await sendInboxMessage(installId, { id: `topup-${jobId}`, title: "Low Balance", text: `Need ${amount} credits. Please top up.` });
    }
    if (error.code === "RATE_LIMITED") {
      await sendInboxMessage(installId, { id: `ratelimit-${jobId}`, title: "Rate Limit Reached", text: "You've hit your usage limit for this period." });
    }
    throw error;
  }
}
...
app.post("/webhooks/officex", async (req, res) => {
  ...
  if (event === "INSTALL") {
    ...
    return res.json({ agent_context: { api_key: user.apiKey, base_url: "https://myapp.com/api/v1" } });
  }
  res.json({ ok: true });
});
```
...
```typescript
app.get("/officex", async (req, res) => {
  const user = await resolveUser(req.query.officex_install_id, req.query.officex_customer_id);
  req.session.user = user;
  req.session.officex = { install_id: req.query.officex_install_id, install_secret: req.query.officex_install_secret };
  res.redirect("/dashboard");
});
```
...
app.post("/api/enrich-lead", async (req, res) => {
  ...
  const reservation = await safeReserve(install_id, install_secret, 5, `enrich-${leadId}-${Date.now()}`);
  if (!reservation.success) return res.status(400).json({ error: reservation.error });

  const enrichedData = await enrichLead(leadId);

  await fetch(`https://cloud.officex.app/v1/reservations/${reservation.reservation_id}/settle`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-officex-install-id": install_id, "x-officex-install-secret": install_secret },
